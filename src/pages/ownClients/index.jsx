...
export const OwnClients = () => {
  ...
  const { status, entities, errorClient } = useSelector(
    (state) => state.clientOwnSlice
  );
  ...
  //   useDispatchAction(ownClientsAction, status);
  ...
  const handleDelete = (id) => {
    dispatch(deleteClient({ id })).then((res) => {
      if (res.payload.status === 200) {
        ...
        dispatch(ownClientsAction({ params: { ...getQueryParams() } }));
      }
    });
  };
